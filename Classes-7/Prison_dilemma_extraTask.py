...
class PrisonDilemma(object):
    ...
    def __init__(self, initial_grid, val_b=1.0, m_games=5):
        ...
        self.unroll_beh_ngb_now = self.create_unroll_beh_ngb_now()
    ...
    def create_unroll_beh_ngb_now(self):
        """
        :return:
        """
        # upper row
        unroll_g_im1_jm1 = np.roll(np.roll(self.beh_grid[1], +2, axis=0), +2, axis=1)  # -1, -1
        unroll_g_im1_j = np.roll(np.roll(self.beh_grid[2], +2, axis=0), 0, axis=1)  # -1, 0
        unroll_g_im1_jp1 = np.roll(np.roll(self.beh_grid[3], +2, axis=0), -2, axis=1)  # -1, +1
        # same row
        unroll_g_i_jm1 = np.roll(np.roll(self.beh_grid[4], 0, axis=0), +2, axis=1)  # 0, -1
        unroll_g_i_jp1 = np.roll(np.roll(self.beh_grid[5], 0, axis=0), -2, axis=1)  # 0, +1
        # lower row
        unroll_g_ip1_jm1 = np.roll(np.roll(self.beh_grid[6], -2, axis=0), +2, axis=1)  # +1, -1
        unroll_g_ip1_j = np.roll(np.roll(self.beh_grid[7], -2, axis=0), 0, axis=1)  # +1, 0
        unroll_g_ip1_jp1 = np.roll(np.roll(self.beh_grid[8], -2, axis=0), -2, axis=1)  # +1, +1

        unroll_beh_ngb = np.array([self.beh_grid[0],
                            unroll_g_im1_jm1, unroll_g_im1_j, unroll_g_im1_jp1,
                            unroll_g_i_jm1, unroll_g_i_jp1,
                            unroll_g_ip1_jm1, unroll_g_ip1_j, unroll_g_ip1_jp1])
        return unroll_beh_ngb
    ...
    def set_beh_after_play(self):
        ...
        for i in range(0, self.n_ngb):
            ...
            # condition to win
            do_I_won_con1 = np.where(np.logical_and(self.beh_grid[i] == 'C', self.unroll_beh_ngb_now[i] == 'C'), 1, 0)
            do_I_won_con2 = np.where(np.logical_and(self.beh_grid[i] == 'D', self.unroll_beh_ngb_now[i] == 'C'), 1, 0)
            ...
            self.beh_grid[i] = np.where(self.str_grid == 'Tit',
                                        self.unroll_beh_ngb_now[i].copy(),
                                        self.beh_grid[i])
    ...
    def do_one_step(self):
        ...
        for i in range(0, self.m_games):
            # print("self.beh_grid")
            # pprint(self.beh_grid)

            self.beh_ngb_now = self.create_beh_ngb_now()  # How look new neighbour behaviour
            self.unroll_beh_ngb_now = self.create_unroll_beh_ngb_now()
            # print("self.beh_ngb_now")
            # pprint(self.beh_ngb_now)
            # print("self.unroll_beh_ngb_now")
            # pprint(self.unroll_beh_ngb_now)

            self.create_payoff_grid()  # Playing games: payoffs
            # print("self.payoff_grid")
            # pprint(self.payoff_grid)
