...
init_lattices = 5  # initial lattices
output_plots_path = currentdir + '/vol2_Test1'
# make directory
...
NumsOfPops = 60  # numbers of population, which we consider
...
for pop in range(0, NumsOfPops):
    ...
    ChessEvolution.update_initial_lattice(n_size=n_size, init_lattices=init_lattices)
    ...
    if pop == NumsOfPops - 1:
        # last chromosomes
        last_chromes_map = new_chromes_map
...
for i in range(0, Nelem):
    ...
    # binaryStr
    binaryStr = last_chromes_map[i]["binaryStr"]
    items_str_list = np.append(items_str_list, binaryStr)
